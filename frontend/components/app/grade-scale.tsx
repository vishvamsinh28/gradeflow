...
import type { Classroom, GradeBand } from "@/lib/types";

const SUGGESTED: GradeBand[] = [
  { label: "A", min: 80 },
  { label: "B", min: 65 },
  { label: "C", min: 50 },
  { label: "D", min: 35 },
 ...
 */
export function GradeScaleCard({ classroom }: { classroom: Classroom }) {
  const toast = useToast();
  const [bands, setBands] = useState<GradeBand[]>(classroom.grade_scale ?? []);
  ...
  const descending = bands.every(
    (band, index) => index === 0 || band.min < bands[index - 1].min,
  );

  async function save(next: GradeBand[]) {
    setSaving(true);
    try {
      await updateClassroom(classroom.id, { grade_scale: next });
    ...
    <section>
      <div className="mb-2.5 flex items-center justify-between">
        ...
        {bands.length === 0 ? (
          ...
          <button
            onClick={() => setBands([...bands, { label: "", min: 0 }])}
              ...
              {bands.map((band, index) => (
                <li key={index} className="group/band flex items-center gap-2 px-3 py-2">
                  <Input
                    ...
                    onChange={(event) => {
                      const next = [...bands];
                      next[index] = { ...band, label: event.target.value };
                      setBands(next);
                    }}
            ...
            <div className="flex items-center gap-2 border-t border-line px-3 py-2">
              ...
              <div className="ml-auto flex items-center gap-1.5">
                ...
                <Button
                  ...
                  onClick={() => void save(bands.map((b) => ({ ...b, label: b.label.trim() })))}
                >
                  Save
                </Button>
              </div>
