...
export function AddStudentsSheet({
  ...
  const [review, setReview] = useState<ParsedStudent[] | null>(null);
          ...
          {mode === "paste" ? (
            <div>
              ...
              <p className="mt-2 text-[12.5px] text-ink-3">
                ...
                <span className="text-ink-2">{pluralize(pastePreview.length, "name")}</span> detected
                so far.
              </p>
            </div>
          ) : null}
...
function ReviewTable({
  ...
  note: string;
  existing: Set<string>;
  onChange: (students: ParsedStudent[]) => void;
  firstCode: number;
}) {
  ...
  return (
    <div>
              ...
              <tr key={index} className="group/row">
                ...
                <td className="border-b border-line py-1 pr-3">
                  <input
                    ...
                    onChange={(event) => {
                      const next = [...students];
                      next[index] = { ...student, name: event.target.value };
                      onChange(next);
                    }}
                    className={cx(
                ...
                <td className="border-b border-line py-1 pr-3">
                  <input
                    ...
                    onChange={(event) => {
                      const next = [...students];
                      next[index] = { ...student, rollNo: event.target.value };
                      onChange(next);
                    }}
      ...
      <div className="px-5 py-3">
        <button
          onClick={() => onChange([...students, { name: "" }])}
