            ...
            grouped.map((entry) => (
              <div key={entry.group} className="mb-1 last:mb-0">
                ...
                {entry.items.map(({ command, index }) => (
                  ...
                  >
                    <span className={index === active ? "text-accent" : "text-ink-3"}>{command.icon}</span>
                    <span className="flex-1 truncate text-[13.5px] font-medium text-ink">{command.label}</span>
                    {command.hint ? (
                      <span className="shrink-0 truncate text-[12px] text-ink-3">{command.hint}</span>
                    ) : null}
                  </button>
                ))}
              </div>
