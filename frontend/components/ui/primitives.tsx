...
type ButtonVariant =
  ...
  | "accent"
  | "danger"
  /* For the dark hero sections, where the page theme does not apply. */
  | "onDark"
  | "onDarkOutline";
...
export function Field({
  ...
  return (
    <label className={cx("block", className)}>
      ...
      {children}
      {hint ? <span className="mt-1.5 block text-[12.5px] leading-snug text-ink-3">{hint}</span> : null}
    </label>
  );
}
...
type BadgeTone = "neutral" | "accent" | "warn" | "danger" | "muted";
...
export function Avatar({ name, size = 26 }: { name: string; size?: number }) {
  ...
  const text =
    parts.length > 1 ? (parts[0][0] + parts[parts.length - 1][0]).toUpperCase() : name.slice(0, 2).toUpperCase();
  let hash = 0;
...
export function EmptyState({
  ...
  return (
    <div className={cx("flex flex-col items-center px-6 py-14 text-center", className)}>
      ...
      {description ? (
        <p className="mt-1.5 max-w-[42ch] text-[13.5px] leading-relaxed text-ink-3">{description}</p>
      ) : null}
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      role="tablist"
      className={cx("inline-flex items-center gap-0.5 rounded-lg border border-line bg-surface-2 p-0.5", className)}
    >
      {options.map((option) => {
