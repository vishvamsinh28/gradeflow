...
const HEADER_HINTS = /^(name|student|student name|full name|roll|roll no|roll number|sr|sr no|s\.no|no|id)$/i;
 ...
 */
export function parseRoster(raw: string): ParsedStudent[] {
  const lines = raw
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  const header = hasHeader ? rows[0].map((cell) => cell.toLowerCase()) : null;
  const body = hasHeader ? rows.slice(1) : rows;

  const nameIndex = header
    ? header.findIndex((cell) => /name|student/.test(cell))
    : -1;
  const rollIndex = header
    ? header.findIndex((cell) => /roll|^sr|s\.no|^no$|^id$/.test(cell))
    : -1;
  ...
  for (const cells of body) {
    ...
    seen.add(key);
    students.push({ name, ...(rollNo ? { rollNo } : {}) });
  }
