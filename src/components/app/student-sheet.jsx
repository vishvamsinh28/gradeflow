...
export function StudentSheet({ student, classroom, onClose }) {
  ...
  const rows = student
    ? [...classroom.tests]
        ...
        .map((test) => {
          ...
          const absent = attendanceOf(classroom.attendance, test.id, student.id) === "absent";
          const percent =
  ...
  return (
    <Sheet
      ...
      description={
        student
          ? `${student.code}${student.roll_no ? ` · Roll ${student.roll_no}` : ""}`
          : undefined
      }
      width={540}
      footer={
                ...
                return (
                  <li key={test.id} className="flex items-center gap-3 px-5 py-2.5">
                    ...
                    {absent ? (
                      ...
                      <Badge tone="muted">{submission ? "Awaiting grading" : "No answer"}</Badge>
                    ) : (
